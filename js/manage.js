...
class ManageInvestments {
    ...
    loadInvestments() {
        ...
        if (existing.length === 0 && placeholderSymbols.length > 0) {
            localStorage.setItem('stocks', JSON.stringify(placeholderSymbols));
        }

        const normalizedStocks = JSON.parse(localStorage.getItem('stocks') || '[]');
        const combinedSymbols = Array.from(new Set([...normalizedStocks, ...placeholderSymbols]));

        if (combinedSymbols.length !== normalizedStocks.length) {
            localStorage.setItem('stocks', JSON.stringify(combinedSymbols));
        }

        this.investments = combinedSymbols.map(symbol => ({
            symbol,
    ...
    setupEventListeners() {
        ...
        if (symbolInput) symbolInput.addEventListener('keypress', (e) => {
        ...
        if (resetBtn) {
            resetBtn.addEventListener('click', () => {
                const confirmed = confirm('Reset all local dashboard data? This clears symbols, portfolio, theme, and cached data.');
                if (!confirmed) return;
        ...
        reader.onload = (e) => {
            try {
                ...
                if (file.name.endsWith('.json')) {
                    ...
                    newSymbols = e.target.result.split('\n')
