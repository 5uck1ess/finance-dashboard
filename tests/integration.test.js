...
describe('Finance Dashboard Integration Flows', () => {
  ...
  test('updatePortfolioSummary aggregates cached holdings into DOM totals', () => {
    ...
    expect(totals['total-pl'].innerHTML).toContain('+$750.00');
    expect(totals['best-performer'].innerHTML).toContain('AAPL');
  });
  ...
  test('updatePortfolioSummary uses local crypto cache entries', () => {
    ...
    expect(totals['total-pl'].innerHTML).toContain('+$40000.00');
    expect(totals['best-performer'].innerHTML).toContain('BTC');
  });
  ...
  describe('Snapshot hydration', () => {
    ...
    test('hydrates dashboard from snapshot payload', async () => {
      ...
      const snapshotPayload = {
        stocks: {
          results: [
            { success: true, data: { symbol: 'AAPL', price: 150, changesPercentage: 1.5, name: 'Apple', provider: 'finnhub' } },
            { success: true, data: { symbol: 'SPY', price: 410, changesPercentage: 0.5, name: 'SPDR', provider: 'finnhub' } }
          ],
          fetchedAt: '2024-01-01T10:00:00Z'
        },
        crypto: {
          results: [
            { success: true, data: { symbol: 'BTC', price: 30000, changePercent: -2, provider: 'coingecko' } }
          ],
    ...
    test('preserves cached analyst details when snapshot omits them for tracked symbols', async () => {
      ...
      const snapshotPayload = {
        stocks: {
          results: [
            { success: true, data: { symbol: 'AAPL', price: 155, changesPercentage: 1.5, name: 'Apple', provider: 'finnhub' } }
          ],
    ...
    test('ignores snapshot entries not tracked by user', async () => {
      ...
      const snapshotPayload = {
        ...
        crypto: {
          results: [
            { success: true, data: { symbol: 'BTC', price: 30000, provider: 'coingecko' } }
          ],
          fetchedAt: '2024-01-01T10:01:00Z'
        }
      };
      ...
      expect(dashboard.renderStockCard).toHaveBeenCalledWith(
