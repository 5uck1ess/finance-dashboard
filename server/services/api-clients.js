// API Client Classes for Finance Dashboard (Server-Side CommonJS)
...
class FinancialModelingPrepClient {
  ...
  async getQuoteShortFallback(symbol) {
    ...
    try {
      ...
      throw new Error(`FMP fallback error for ${symbol}: ${error.message}`);
    }
  }
...
class CoinGeckoClient {
  ...
  async getCryptoId(symbol) {
    ...
    try {
      ...
      const coin = data.coins.find(
        (c) => c.symbol.toLowerCase() === symbol.toLowerCase() && c.market_cap_rank && c.market_cap_rank <= 100
      );
  ...
  async getCryptoData(symbol) {
    ...
    try {
      ...
      return {
        ...
        sparkline7d: data.market_data?.sparkline_7d?.price || null,
      };
    } catch (error) {
  ...
  async getMultipleCryptoData(symbols) {
    try {
      ...
      const url = `${this.baseUrl}/coins/markets?vs_currency=usd&ids=${ids.join(',')}&order=market_cap_desc&per_page=${symbols.length}&page=1&sparkline=true&price_change_percentage=24h`;
        ...
        if (coinData) {
          return {
            ...
            data: {
              ...
              sparkline7d: coinData.sparkline_in_7d?.price || null,
            },
          };
        } else {
